...
// Compare HH:MM strings with ±6 hour tolerance
function isTimeClose(time1, time2, marginMinutes = 360) {
...
router.get("/discover/:tripId", protect, async (req, res) => {
  try {
    const baseTrip = await Trip.findById(req.params.tripId).populate(
      "user",
      "name declaredGender genderPreference"
    );
    ...
    const mutualGender = (tripA, tripB) => {
      if (!tripA?.user || !tripB?.user) {
        // console.warn("Skipping gender check: user missing", {
      ...
      // console.log(
      //   `Gender Match: A [${genderA}] prefers [${prefA}] ⇄ B [${genderB}] prefers [${prefB}] => ${isMatch}`
      // );
      return isMatch;
    };
    ...
    for (const baseLeg of baseLegs) {
      ...
      for (const otherTrip of allTrips) {
        ...
        const directMatch = otherLegs.some((otherLeg) =>
          isLegMatch(baseLeg, otherLeg)
        );
  ...
  try {
    ...
    for (const conn of connections) {
      ...
      const latest = new Date(
        Math.max(lastDateTimeA.getTime(), lastDateTimeB.getTime())
      );
...
function getLatestDateTime(trip) {
  ...
  if (trip.date && trip.time) {
    times.push(
      new Date(`${trip.date.toISOString().split("T")[0]}T${trip.time}`)
    );
  }
  ...
  if (Array.isArray(trip.legs)) {
    for (const leg of trip.legs) {
      if (leg.date && leg.time) {
        times.push(
          new Date(`${leg.date.toISOString().split("T")[0]}T${leg.time}`)
        );
      }
    }
  }
